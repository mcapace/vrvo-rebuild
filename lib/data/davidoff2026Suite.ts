  ...
  return DAVIDOFF_2026_PLACEMENTS.map((p) => {
    ...
    const tradeDesk = {
      ...
      formatDelivery: buildFormatDelivery(
        p.formats.map((f, i) => ({
          format: f,
          weight: 1 / p.formats.length,
          ctrMultiplier: 1.0 + (i === 0 ? 0.1 : -0.05),
        })),
        p.deliveredImps,
        p.clicks,
      ),
...
const tradeDeskComprehensive = {
  ...
  formatDelivery: buildFormatDelivery(
    [
      { format: 'High-Impact Pop-up / Interstitial', weight: 0.185, ctrMultiplier: 1.7 },
      { format: '300x250 Medium Rectangle', weight: 0.28, ctrMultiplier: 0.8 },
      { format: '728x90 Leaderboard', weight: 0.22, ctrMultiplier: 0.7 },
      { format: '300x600 Half-Page', weight: 0.17, ctrMultiplier: 0.95 },
      { format: '970x250 Billboard', weight: 0.07, ctrMultiplier: 0.75 },
      { format: 'Editorial Sponsored Content & Native', weight: 0.075, ctrMultiplier: 2.4 },
    ],
    TOTAL_DELIVERED_IMPS,
    TOTAL_CLICKS,
  ),
